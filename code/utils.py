...
def preprocess_features(features, block_size):
    ...
    for feature in features:
        ...
        pad0 = 32 - feature.shape[0]
        if pad0 < 0:
            new_features.append(feature[:32, :])
        else:
            new_feature = np.pad(feature, ((0, pad0), (0, 0)), mode='constant')
            new_features.append(new_feature)

    new_features = np.array(new_features)
    pad = max_length - new_features.shape[0]  # padding for each epoch
    new_features = np.pad(new_features, ((0, pad), (0, 0), (0, 0)), mode='constant')
    return new_features
...
def preprocess_adj(adj_list, block_size):
    ...
    return adj_list_new, mask  # coo_to_tuple(sparse.COO(np.array(list(adj)))), mask
